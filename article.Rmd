...
To begin, let us start by reviewing a general panel model [@Bollen2010], also referred to as the 'unobserved effects model' [@Wooldridge2012]
\begin{align}
y_{it} & = \bm{x}_{it}\bm{\beta} + \bm{z}_{i}\bm{\gamma} + \alpha_{i} + \varepsilon_{it} \label{eq:gpm}
\end{align}
where $y_{it}$ is the dependent variable for unit $i$ at time $t$, $\bm{x}_{it}$ is a $1 \times K$ vector of time-varying covariates (which could include a constant) linked to the dependent variable by the $K \times 1$ vector of coefficients $\bm{\beta}$, $\bm{z}_{i}$ is a $M \times 1$ vector of time-invariant covariates linked to the dependent variable by the $M \times 1$ vector of coefficients in $\bm{\gamma}$, $\alpha_{i}$ is the unit-specific effect that represents the combined effect of all unobserved unit-specific variables affecting the dependent variable and $\varepsilon_{it}$ is the idiosyncratic error.   

We can make stating some of the model assumptions easier by rewriting the model in matrix notation
\begin{align}
...
We typically make the following assumptions [see, e.g., @Wooldridge2002; @Schmidheiny2019]: 
...
\end{pmatrix}
\end{align}
where $\sigma^{2}_{\nu} = \sigma^{2}_{\alpha} + \sigma^{2}_{\varepsilon}$. This means that in the conditional covariance matrix of the errors, given the time-varying and -invariant covariates, units over time will be correlated due to the individual effects. Keep the covariance structure of the errors in mind as it will help make sense of the use of latent variables to decompose the dependent variable into between- and within-variance components. 

## Fixed effects 

What if the individual effects are independent of the observed time-varying covariates, i.e., $\E[\alpha_{i}|\bm{X}_{i}] \ne 0$? Then grouping them in with the error will cause $\bm{\beta}$ to be biased [@Wooldridge2002; @Wooldridge2012]. In order to drop assumptions involving the individual effects, a number of methods are available (e.g., differencing, least squares dummy variable regression), but the most common approach involves \textit{demeaning} the equation [@Bruederl2015]. Demeaning involves subtracting the per-unit, over-time average from each of the model terms, i.e.,
\begin{align}
(y_{it} - \bar{y}_{i}) & = (\bm{x_{it}} - \bar{\bm{x}}_{i})\bm{\beta} + (\bm{z}_{i} - \bar{\bm{z}}_{i})\bm{\gamma} + (\alpha_{i} - \bar{\alpha}_{i}) + (\varepsilon_{it} - \bar{\varepsilon}_{i}) \\
...
And in fact this is exactly how a latent variable is used to account for time-invariant unobserved heterogeneity. The dependent variable at each timepoint is regressed onto the latent variable, see Figure \ref{fig:fesem}. Here, the regression weights or 'factor loadings' are fixed to one to represent our assumption that the effect of the time-invariant unobserved heterogeneity is constant over time. It also means that the estimated variance of the latent variable is equal to the average covariance between the dependent variable over time. If $y_{it} = \alpha_{i} + \varepsilon_{it}$ is the true data generating process, then the covariance between all timepoints is constant and thus equal to the average covariance between all timepoints. 
...
\end{pmatrix}
\end{align}
which shows that if our assumption of constant covariances between the dependent variable across time holds, the estimated variance of $\alpha$ is just what it should be: the average covariance between units of $y$ over time. Once we add in observed covariates, the estimated covariance of $\alpha$ then become the \textit{conditional} covariance of $y$ over time, given those covariates.  
