...
If we are willing to make some assumptions about the distribution of the individual effects and, more importantly, their lack of a relationship with the observed time-varying covariates, we are left with the random effects model [@Wooldridge2002; @Wooldridge2012; @Schmidheiny2019; @Skrondal2004]. Namely, we assume the individual effects are normally distributed, with a constant (homoscedastic) variance conditional on the model covariates, i.e., $\alpha_{i}|\bm{X}_{i}, \bm{z}_{i} \sim N(0, \sigma^{2}_{\alpha})$.^[As long as $\bm{X}_{i}$ contains a constant, the assumption that $\alpha_{i}$ has a mean of zero is unproblematic [@Wooldridge2012].] Further, we assume that the expectation of the individual effects conditional on the model covariates is zero, i.e., $\E[\alpha_{i}|\bm{X}_{i},\bm{z}_{i}] = 0$, which implies they are also uncorrelated. 
...
\begin{align}
\Var(\bm{\nu} | \bm{X}, \bm{z}) & = \bm{\Omega}_{\bm{\nu}} = 
\begin{pmatrix}
\bm{\Omega}_{\bm{\nu}_{1}} & \hdots & 0 & \hdots & 0 \\
\vdots & \ddots & \vdots & \ddots & \vdots \\
0 & \hdots & \bm{\Omega}_{\bm{\nu}_{i}} & \hdots & 0 \\
\vdots & \ddots & \vdots & \ddots & \vdots \\
0 & \hdots & 0 & \hdots & \bm{\Omega}_{\bm{\nu}_{N}}
\end{pmatrix}
\end{align}
with typical elements 
\begin{align}
\Var(\bm{\nu}_{i} | \bm{X}_{i}, \bm{z}_{i}) & = \bm{\Omega}_{\bm{\nu}_{i}} = 
\begin{pmatrix}
...
\begin{align}
\bm{b} & = \bm{A}\bm{x} \\
(\bm{A}^{\intercal}\bm{A})^{-1}\bm{A}^{\intercal}\bm{b} & = (\bm{A}^{\intercal}\bm{A})^{-1}\bm{A}^{\intercal}\bm{A}\bm{x} \\
(\bm{A}^{\intercal}\bm{A})^{-1}\bm{A}^{\intercal}\bm{b} & = \bm{x} \\
\begin{pmatrix}
