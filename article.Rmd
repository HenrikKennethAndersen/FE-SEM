...
To begin, let us start by reviewing a general panel model [@Bollen2010], also referred to as the 'unobserved effects model' [@Wooldridge2012] (we will return to this model in the [online section](https://github.com/henrik-andersen/FE-SEM/blob/master/extensions.pdf) of the article when we discuss loosening assumptions)
\begin{align}
y_{it} & = \bm{x}_{it}\bm{\beta} + \bm{z}_{i}\bm{\gamma} + \alpha_{i} + \varepsilon_{it} \label{eq:gpm}
\end{align}
where $y_{it}$ is the dependent variable for unit $i, \ i = 1, ..., N$ at time $t, \ t = 1, ..., T$, $\bm{x}_{it}$ is a $1 \times K$ vector of time-varying covariates (which could include a constant) linked to the dependent variable by the $K \times 1$ vector of coefficients $\bm{\beta}$, $\bm{z}_{i}$ is a $1 \times M$ vector of time-invariant covariates linked to the dependent variable by the $M \times 1$ vector of coefficients in $\bm{\gamma}$, $\alpha_{i}$ represents the combined effect of all unobserved unit-specific variables affecting the dependent variable and $\varepsilon_{it}$ is the idiosyncratic error.   
...
We typically make the following assumptions [see, e.g., @Wooldridge2002; @Schmidheiny2019] about this model:

- Linearity: the model is linear in its parameters and $\E[\varepsilon_{it}] = 0$ and $\E[\alpha_{i}] = 0$. 
- Independence: the observations are independent across individuals, but not necessarily across time. 
- Strict exogeneity: $\E[\varepsilon_{it} | \bm{X}_{i}, \bm{z}_{i}, \alpha_{i}] = 0$, i.e., the idiosyncratic error at each point in time is mean independent of $\bm{X}_{i}$ at \textit{all} points in time, $t = 1, ..., T$, as well as $\bm{z}_{i}$ and $\alpha_{i}$ . 
- Error variance: homoscedastic and non-serially correlated errors, i.e., $\Var(\varepsilon_{i}|\bm{X}_{i},\bm{z}_{i}, \alpha_{i}) = \sigma^{2}_{\varepsilon} \bm{I}$. 

## Random effects {#re}

If we are willing to make some assumptions about the distribution of the individual effects and their relationship with the observed time-varying covariates, we are left with the random effects model [@Wooldridge2002; @Wooldridge2012; @Schmidheiny2019; @Skrondal2004]. Namely, we assume the individual effects are normally distributed, with a constant (homoscedastic) variance conditional on the model covariates, i.e., $\alpha_{i}|\bm{X}_{i}, \bm{z}_{i} \sim N(0, \sigma^{2}_{\alpha})$.^[As long as $\bm{X}_{i}$ contains a constant, the assumption that $\alpha_{i}$ has a mean of zero is unproblematic [@Wooldridge2012].] Further, we assume that the expectation of the individual effects conditional on the model covariates is zero, i.e., $\E[\alpha_{i}|\bm{X}_{i},\bm{z}_{i}] = 0$, which implies they are also uncorrelated. 

The random effects model works by defining a composite error term: $\nu_{it} = \alpha_{i} + \varepsilon_{it}$ and rewriting the model in Equation \eqref{eq:gpm} as 
\begin{align}
y_{it} & = \bm{x}_{it}\bm{\beta} + \bm{z}_{i}\bm{\gamma} + \nu_{it}.
\end{align}
By adding the individual effects to the composite error in each time period, the composite error becomes serially correlated over time, to give the following conditional covariance matrix for the errors, 
\begin{align}
\Var(\nu | \bm{X}, \bm{z}) & = \bm{\Omega}_{\nu} = 
\begin{pmatrix}
\bm{\Omega}_{\nu 1} & \hdots & 0 & \hdots & 0 \\
\vdots & \ddots & \vdots & \ddots & \vdots \\
0 & \hdots & \bm{\Omega}_{\nu i} & \hdots & 0 \\
\vdots & \ddots & \vdots & \ddots & \vdots \\
0 & \hdots & 0 & \hdots & \bm{\Omega}_{\nu N}
\end{pmatrix}
\end{align}
with typical elements 
\begin{align}
\Var(\nu_{i} | \bm{X}_{i}, \bm{z}_{i}) & = \bm{\Omega}_{\nu i} = 
\begin{pmatrix}
...
\end{pmatrix}
\end{align}
where $\sigma^{2}_{\nu} = \sigma^{2}_{\alpha} + \sigma^{2}_{\varepsilon}$. This means that in the conditional covariance matrix of the errors, given the time-varying and -invariant covariates, units over time will be correlated due to the individual effects. We should keep the covariance structure of the errors in mind as it will help make sense of the use of latent variables to decompose the dependent variable into between- and within-variance components, discussed below in Section \ref{fe-sem}. 

## Fixed effects {#fe}

What if the individual effects are \textit{note} independent of the observed time-varying covariates, i.e., $\E[\alpha_{i}|\bm{X}_{i}] \ne 0$? Then grouping them in with the error will cause $\bm{\beta}$ to be biased [@Wooldridge2002; @Wooldridge2012]. In order to drop assumptions involving the individual effects, a number of methods are available (e.g., differencing, least squares dummy variable regression), but the most common approach is to \textit{demean} the equation [@Bruederl2015]. Demeaning involves subtracting the per-unit, over-time average from each of the model terms, i.e.,
\begin{align}
...
\ddot{y}_{it} & = \bm{\ddot{x}}_{it}\bm{\beta} + \ddot{\varepsilon}_{it} \label{eq:fe}
\end{align}
where, for example, $\bar{y}_{i} = T^{-1}\sum_{i = 1}^{T}y_{it}$ (and the other over-time means are calculated analogously), and the variables with the dots above them represent the demeaned versions. 
Because the average of something that does not change is that thing itself, the individual effects, along with any time-invariant predictors, get wiped out by the demeaning. This means that no assumptions about the relatedness of the model covariates and the unit-specific portion of the error are needed. The unbiasedness of the estimate is related solely to the strict exogeneity assumption imposed on the idiosyncratic errors, i.e., $\E[\ddot{\varepsilon}_{it}|\bm{\ddot{x}}_{it}] = \E[\ddot{\varepsilon}_{it}] = 0$ which also implies $\E[\bm{\ddot{x}}_{is}^{\intercal}\ddot{\varepsilon}_{it}] = \bm{0}, \ \forall \ s, t = 1, ..., T$ [@Bruederl2015; @Wooldridge2002]. The downside to this approach is that no time-invariant predictors can be included in the model. In the [online section](https://github.com/henrik-andersen/FE-SEM/blob/master/extensions.pdf), it will be discussed how to get around this restriction using SEM. 
...
We first need to convert the data from stacked, long-format vectors of length $NT$ into $T$ individual vectors of length $N$. To see why this is necessary, consider what effect this has on the vector of responses $y_{it}$. Let us, for a minute ignore any covariates and focus just on the dependent variable (a so-called 'intercept-only' model) so that we have $y_{it} = \alpha_{i} + \varepsilon_{it}$. When we convert the data to wide-format, we get $T$ individual equations,
\begin{align}
y_{1i} & = \alpha_{i} + \varepsilon_{1i} \\
y_{2i} & = \alpha_{i} + \varepsilon_{2i} \\
 & \vdots \\
y_{Ti} & = \alpha_{i} + \varepsilon_{Ti}. \label{eq:wide}
\end{align}
Because the idiosyncratic errors are assumed to be uncorrelated across units and across time, the covariance between any two of the new wide vectors $\Cov(y_{ti},y_{si}) = \Var(\alpha_{i}), \ t \ne s$. Otherwise, when $t = s$, the variance is $\Var(\alpha_{i}) + \Var(\varepsilon_{it})$. This is exactly the structure we saw above in a typical element of $\bm{\Omega}_{\nu}$. 

And in fact this is exactly how a latent variable is used to account for time-invariant unobserved heterogeneity. The dependent variable at each timepoint is regressed onto the latent variable, see Figure \ref{fig:fesem}. Here, the regression weights or 'factor loadings' are fixed to one to represent our assumption that the effect of the time-invariant unobserved heterogeneity is constant over time. It also means that the estimated variance of the latent variable is equal to the average covariance between the dependent variable over time. If $y_{it} = \alpha_{i} + \varepsilon_{it}$ is the true data generating process, then the expected covariances between all timepoints are constant and thus equal to the average covariance between all timepoints. 

To show this, consider the following matrix equation of the nonredundant covariances in a three-wave intercept-only model that just restates Equation \eqref{eq:wide}, and which we can solve easily with least squares:
\begin{align}
\bm{b} & = \bm{A}\bm{x} \\
\begin{pmatrix}
\Var(y_{1}) \\
...
\end{pmatrix}
\end{align}
where $\psi = \Var(\alpha)$, $\phi_{t} = \Var(\varepsilon_{t})$. We can solve this equation like so
\begin{align}
(\bm{A}^{\intercal}\bm{A})^{-1}\bm{A}^{\intercal}\bm{b} = 
\begin{pmatrix}
\psi \\
\phi_{1} \\
\phi_{2} \\
...
\end{pmatrix} = 
\begin{pmatrix}
.33\Cov(y_{2},y_{1}) + .33\Cov(y_{3},y_{1}) + .33\Cov(y_{3},y_{2}) \\
\Var(y_{1}) - \psi \\
\Var(y_{2}) - \psi \\
\Var(y_{3}) - \psi
\end{pmatrix}.
\end{align}
In other words, $\psi = .33\Cov(y_{2},y_{1}) + .33\Cov(y_{3},y_{1}) + .33\Cov(y_{3},y_{2})$ is the same as $\psi = \frac{1}{3} 3\Cov(y_{t},y_{s}) = \Cov(y_{t},y_{s})$ where $\Cov(y_{t},y_{s})$ is the covariance between any two columns of the wide-format dependent variable. This shows that if our assumption of constant covariances between the dependent variable across time holds, the estimated variance of $\alpha$ is just what it should be: the average covariance between units of $y$ over time. Once we add in observed covariates, the estimated covariance of $\alpha$ then become the \textit{conditional} covariance of $y$ over time, given those covariates.  
...
Let us, however, make things more concrete and take a look at a simple, three-wave version of the typical FE-SEM using this notation (shown graphically in Figure \ref{fig:fesem}). For that, we have:
\begin{align}
\begin{split}
\bm{y}^{+} & = \bm{\Lambda_{y}}^{+}\bm{\eta}^{+} \\
\begin{pmatrix}
