...
\end{align}
where $\theta$ is just the residual of a regression in which the true underlying variable is regressed on all other covariates. In this case, we need to regress $x_{t}$ on $\alpha_{1}$ and $\alpha_{2}$ for: $x_{t} = \tau + \gamma_{1}\alpha_{1} + \gamma_{2}\alpha_{2} + \theta_{t}$ where $\tau$ is the intercept, and $\gamma_{1}$, $\gamma_{2}$ are the regression coefficients and $\theta_{t}$ is the residual [@Wooldridge2009, p. 318--320].
